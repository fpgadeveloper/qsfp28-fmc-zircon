...
**Status: two ports on the VCK190, bare-metal only** (both QSFP28 ports of the card on FMCP1; no
Linux image). ZCU106 and KCU116 targets are planned. Zircon is under active development upstream; this design pins Taxi at `cc70b27` and works
around a UDP checksum issue of its deparser (see the docs).
...
* Vivado 2025.2, **Enterprise** edition (the XCVC1902 device), with the no-cost **MRMAC license**
* Vitis 2025.2
* [2x QSFP28 FMC]
* One of the target platforms listed below
* For the loopback test: one QSFP28 100G cable (DAC, AOC or a pair of optical modules with a fibre)
  between port 0 and port 1 of the card — nothing else; or a QSFP28 loopback plug for one port
...
The board is loaded with the resulting `BOOT.BIN`, from a microSD card or over JTAG. With a
