...
int vadj_enable(vadj_voltage_t voltage)
{
	...
	case VADJ_1V2:
		...
		break;
	default:
		con_printf("VADJ: unknown voltage setting\r\n");
		return XST_FAILURE;
	}
...
#endif
	if (cfg == NULL) {
		con_printf("VADJ: I2C0 config lookup failed\r\n");
		return XST_FAILURE;
	}

	status = XIicPs_CfgInitialize(&iic, cfg, cfg->BaseAddress);
	if (status != XST_SUCCESS) {
		con_printf("VADJ: I2C0 init failed\r\n");
		return XST_FAILURE;
	}
	...
	if (status != XST_SUCCESS) {
		con_printf("VADJ: I2C mux select failed\r\n");
		return XST_FAILURE;
	}
	...
	for (int i = 0; i < num_writes; i++) {
		...
		if (status != XST_SUCCESS) {
			con_printf("VADJ: write to reg 0x%02x failed\r\n",
				   writes[i].reg);
			return XST_FAILURE;
		}
	}

	con_printf("VADJ: %s enabled successfully\r\n", label);
	return XST_SUCCESS;
}
...
{
	(void)voltage;
	con_printf("WARNING: VADJ for this board has not been enabled!\r\n");
	return XST_SUCCESS;
}
